...
Eine   Solarzelle  wird   h\"aufig   durch  eine   Stromquelle  mit   parallel
geschalteter  Diode,  wie  in Abbildung  \ref{fig:pvcell:1diode}  dargestellt,
modelliert. Je   nach  Modell   kommen   zus\"atzlich   noch  ein   paralleler
Shunt-Widerstand $R_{\mathrm{Sh}}$,  ein Seriewiderstand  $R_{\mathrm{S}}$ und
eine zweite Diode hinzu. Das  Eindiodenmodell ist gem\"ass \cite{pvcell:phang}
bei  niedrigen  Beleuchtungsgraden  ziemlich  ungenau, weshalb  wir  hier  ein
Zweidiodenmodell benutzen werden. Eine der  beiden Dioden modelliert dabei den
S\"attigungsstrom, welcher von Diffusionsprozessen verursacht wird, die zweite
Diode  modelliert den  S\"attigungsstrom,  welcher von  Rekombinationseffekten
verursacht wird \cite{pvcell:masmoudi}.


Zur  Herleitung der  Modellparamater  existieren verschiedene  Verfahren. Eine
M\"oglichkeit  ist das  experimentelle Ausmessen  verschiedener Aspekte  einer
PV-Zelle und  das Herleiten der Modellparameter  aus den Messergebnissen. Dies
ist  jedoch  nicht  immer  praktisch  realisierbar,  da  man  allenfalls  kein
Testexemplar   zur   Verf\"ugung   hat.    Sollen   bei   der   Messung   noch
Fertigungstoleranzen  der   PV-Zellen  ber\"ucksichtigt  werden,   steigt  der
experimentelle  (und   finanzielle)  Aufwand  zus\"atzlich,  da   man  mehrere
Exemplare   kaufen  und   ausmessen  muss. Auch   die  Auswertung   wird  mehr
Arbeit  erfordern,  wenn  man mittels  statistischer  Methoden  verl\"assliche
...
Testexemplaren,  andererseits  ist  davon   auszugehen,  dass  der  Hersteller
mehrere Exemplare ausgemessen  hat und typische Angaben  in den Datenbl\"atten
gemacht werden. Nat\"urlich setzt dies  voraus, dass man den Herstellerangaben
einigermassen  vertraut;  Sicherheit  ist  nur durch  eigene  Verifikation  zu
erlangen.

Das Herleiten der Modellparameter aus den Herstellerangaben ist nicht trivial;
es wird deshalb  an dieser Stelle ein Modell als  Grundlage verwendet, welches
in \cite{pvcell:masmoudi} hergeleitet und verifiziert worden ist.
